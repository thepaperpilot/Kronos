<template>
    <teleport to="#modal-root">
        <transition name="cutscene-modal">
            <div
                class="modal-mask"
                style="z-index: 10; background-color: var(--raised-background); top: 50px"
            >
                <div class="cutscene-positioner">
                    <div class="modal-container cutscene-container">
                        <div class="paper-background" />
                        <div class="content">
                            <component :is="stageComp" />
                            <div v-if="captionComp" class="caption">
                                <component :is="captionComp" />
                            </div>
                        </div>
                        <button @click="emit('next')" class="button next">Next</button>
                    </div>
                    <svg style="display: none">
                        <filter id="wavy2">
                            <feTurbulence
                                x="-50vw"
                                y="-50vh"
                                width="200vw"
                                height="200vh"
                                baseFrequency="0.015"
                                numOctaves="10"
                                :seed="seedToUse"
                            ></feTurbulence>
                            <feDisplacementMap in="SourceGraphic" scale="20" />
                        </filter>
                    </svg>
                </div>
            </div>
        </transition>
    </teleport>
</template>

<script setup lang="ts">
import { CoercableComponent } from "features/feature";
import { computeComponent, computeOptionalComponent } from "util/vue";
import { ref, toRef, watch } from "vue";
const props = defineProps<{
    stage: CoercableComponent;
    caption?: CoercableComponent;
}>();
const stage = toRef(props, "stage");
const caption = toRef(props, "caption");

const emit = defineEmits<{
    (event: "next"): void;
}>();

const seedToUse = ref<number>(1);
watch([stage, caption], () => seedToUse.value++);

const stageComp = computeComponent(stage);
const captionComp = computeOptionalComponent(caption);
</script>

<style scoped>
.cutscene-positioner {
    height: 100%;
    overflow-y: auto;
    display: flex;
}

.modal-container.cutscene-container {
    width: 80vw;
    max-width: 800px;
    max-height: unset;
    position: relative;
    background: none;
    border: none;
    color: var(--feature-foreground);
}

.paper-background {
    position: absolute;
    z-index: -1;
    width: calc(100% - 40px);
    height: calc(100% - 87px);
    box-shadow: 0px 3px 8px black, 0 0 125px #603204 inset;
    background: #d59d6d;
    filter: url(#wavy2);
    background-image: url(data:image/png;base64,);
}

.content {
    width: calc(100% - 40px);
    padding: 20px;
    display: flex;
    flex-flow: row wrap;
}

.caption {
    width: 100%;
    margin-top: 10px;
}
.caption * {
    font-size: x-large;
    font-family: "Papyrus";
}

.next {
    font-size: larger;
    margin-right: 0;
    margin-top: 20px;
}
</style>
